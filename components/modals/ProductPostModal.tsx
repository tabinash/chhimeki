...
import { X, Check, Upload, Image as ImageIcon, Loader2, Trash2, MapPin } from "lucide-react";
...
export default function ProductPostModal({ isOpen, onClose, initialData, onSubmit }: ProductPostModalProps) {
    ...
    const updateProductMutation = useUpdateProduct(initialData?.id || 0);
    ...
    useEffect(() => {
        if (isOpen) {
            if (initialData) {
                ...
                setIsNegotiable(initialData.isNegotiable);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!isEditMode) {
            ...
            // Only call if we're actually in edit mode with valid product ID
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-bold text-gray-700 mb-2">
                            Product Images {!isEditMode && <span className="text-red-500">* (1-5 required)</span>}
                        </label>

                        {/* Display Images Grid */}
                        <div className="grid grid-cols-3 gap-3 mb-3">
                            {/* For CREATE mode */}
                            {!isEditMode && imagePreviews.map((preview, index) => (
                            ...
                            {isEditMode && existingImages.map((img) => (
                                <div
                                    key={img.id}
                            ...
                            {/* For EDIT mode - new images */}
                            {isEditMode && newPreviews.map((preview, index) => (
                        ...
                        {/* Upload Button */}
                        {(!isEditMode && imagePreviews.length < 5) || (isEditMode && (existingImages.length - imagesToRemove.length + newFiles.length) < 5) && (
                            <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-200 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
                                ...
                                <input
                                    ...
                                    accept="image/*"
                                    multiple
                                    onChange={isEditMode ? handleNewFilesSelect : handleFilesSelect}
                                />
                            </label>
                        )}
                    </div>
                    ...
                    {/* Location Display (for create mode) */}
                    {!isEditMode && (
                        <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1">
                                    <h4 className="text-sm font-bold text-gray-900 mb-1">Listing Location</h4>
                                    <p className="text-sm text-gray-600">
                                        {user?.palika && user?.district
                                            ? `${user.palika}, ${user.district}`
                                            : "Location not set - please update your profile"}
                                    </p>
                                    <p className="text-xs text-gray-500 mt-1">Buyers from your area will see this listing</p>
                                </div>
                            </div>
                        </div>
                    )}
